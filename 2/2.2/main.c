...
/*
 * Main is just a dummy task for testing the Broadcast function
 */
    ...
    int root, MPI_Comm communicator)
{
    ...
    if(process == root)
    {
        ...
        {
            if(i == root) continue;
            result = MPI_Send(buffer, count, datatype, i, TAG, communicator);
    ...
    {
        result = MPI_Recv(buffer, count, datatype, root, TAG, communicator,
            &status);
